...
- [ ] **Task 1 — Verify runner `internal/verify` (AC: 1, 5)**
  - [ ] `internal/verify/verify.go`: `Run(ctx, dir string, commands []string) (Result, error)` where `Result` holds, per command, the command string, combined output, and exit code/error — plus an overall `AllPassed bool` and the assembled combined-output text. Shell out via `exec.CommandContext(ctx, "sh", "-c", cmd)` with `cmd.Dir = dir`, capturing combined stdout+stderr.
  - [ ] A non-zero command exit is a recorded outcome, **not** a returned error; `Run` returns an error only for an inability to execute (e.g. `sh` missing). Keep it testable as a unit (no FS writes here).
- [ ] **Task 2 — `App.Verify` use-case (AC: 2, 3, 4, 6)**
  - [ ] `internal/app/verify.go`: resolve cwd → require `.ai-session/` (`ErrNoSession`) → `config.Load` → if `cfg.Verify` empty, print guidance and return nil (exit 0) → `verify.Run(ctx, root, cfg.Verify)` → write the combined output to `paths.LatestVerify()` via `WriteAtomic` → append a timestamped entry per command to `paths.CommandLog()` → print a concise pass/fail summary → return an error (so exit is non-zero) iff a command failed.
  - [ ] `command-log.md` append: open-or-create and append (this is the one Session artifact that grows by append rather than full-rewrite; document the choice — a full atomic rewrite of an ever-growing log is wasteful). Keep entries one-per-line-ish: e.g. `<RFC3339 UTC> $ <cmd> (exit N)`.
- [ ] **Task 3 — Command `cmd/aictl/verify.go` (AC: 1)**
  - [ ] `aictl verify` (`cobra.NoArgs`) → `app.Verify`; register on root. Thin `cmd/`.
- [ ] **Task 4 — Tests (AC: 1–6)**
  - [ ] `internal/verify/verify_test.go`: a passing command (`true` / `echo`) and a failing one (`false`) — outputs captured, exit codes recorded, `AllPassed` correct; multiple commands run in order; combined output contains each command's output.
  - [ ] `internal/app/verify_test.go` (`t.Chdir`+`t.TempDir`, after `init`/`start`): `Verify` writes `latest-verify.txt` with the output and appends to `command-log.md`; a failing command → `latest-verify.txt` still written and `Verify` returns an error; no `.ai-session/` → `ErrNoSession`; empty `verify` config → no error, nothing written.
  - [ ] Integration: after `Verify`, `Handoff` output contains the verify text (confirms the 2.2 include-if-present wiring).
